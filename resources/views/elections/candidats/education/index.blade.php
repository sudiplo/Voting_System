<x-defult_layout>
    ...
    <header class="w-full glass px-6 py-4 flex justify-between items-center shadow-xl">
        <div class="flex items-center gap-4">
            <a href="" class="text-2xs text-gray-500 hover:text-blue-500">⌘ Education Management</a>
        </div>
    </header>
    ...
    <div class="mt-5 mb-10 bg-white shadow-lg rounded-xl p-8 border border-gray-200">
        ...
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            ...
            <h1 class="text-4xl font-extrabold text-gray-800 tracking-tight">
                Education <span class="text-indigo-600">Management</span>
            </h1>
            <p class="mt-2 text-gray-500">
                Registe and manage education degrees for candidates.
            </p>
           <!-- Search Bar -->
                {{-- <form method="GET" action="{{ route('districts.index') }}" class="mt-4 flex flex-col sm:flex-row items-start sm:items-center gap-2 max-w-md">
                    <input type="text" name="search" list="districts" value="" placeholder="Search district..."
        ...
        <div id="dropdown" class="hidden mt-6 w-full sm:w-[28rem] rounded-2xl border border-gray-200 bg-white shadow-lg" >
            ...
            <div class="p-5 border-b">
                <h3 class="text-lg font-semibold text-gray-800"> Add New Degree</h3>
                <p class="text-sm text-gray-500"> Enter degree details.</p>
            </div>

            <!-- Form -->
            <form method="POST" action="{{ route('education-degrees.store') }}" class="p-5 space-y-4" onsubmit="return confirm('Are you sure you want to add this degree?')" >
                @csrf
                <!-- Degree Name (English) -->
                <div>
                    ...
                    <input type="text" name="level" placeholder="e.g. Bachelor of Arts" required
    ...
    {{--  --}}
    <div class="mt-10 mb-14 bg-white shadow-xl rounded-2xl border border-gray-200 overflow-hidden">
                        ...
                        @forelse ($edu as $degree)
                            <tr class="group hover:bg-indigo-50 transition">
                                ...
                                <td class="px-6 py-4 text-right space-x-2 opacity-90 group-hover:opacity-100">                    
                                    ...
                                    <form action="{{ route('education.destroy',$degree->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button
                                            type="submit"
