...
public function sendOtp(Request $request)
{
    ...
    //         // Get the current encrypted vote (raw ciphertext from DB)
    //         $currentCipher = $candidate->getOriginal('vote');
    //         // If for any reason it's null, start from encrypted 0
    //         if (is_null($currentCipher)) {
    //             $currentCipher = $paillier->encrypt(0);
    //         }

    //         // Encrypt the number 1
    //         $oneCipher = $paillier->encrypt(1);

    //         // Homomorphic addition: newCipher = currentCipher + 1 (encrypted)
    //         $newCipher = $paillier->add($currentCipher, $oneCipher);

    //         // Store the new ciphertext (the model does NOT cast this field)
    //         $candidate->vote = $newCipher;
    //         $candidate->save();

    //         // Record the vote in your `votes` table (metadata only, not used for tally)
    //         vote::create([
    //             'user_id'      => Auth::user()->id,
    //             'candidate_id' => $candidate->id,
    //             'election_id'  => $candidate->election,
